 ...
 * All rights reserved.
 *
 * This source code is licensed under the BSD-style.css license found in the
 ...
 * @param {string} key style.css name, eg: transitionDuration
 * @return {string} style.css name prefixed with `prefix`, properly camelCased, eg:
...
/**
 * Support style.css names that may come passed in prefixed by adding permutations
 * of vendor prefixes.
 */
...
/**
 * Most style.css properties can be unset by doing .style.css[prop] = '' but IE8
 * doesn't like doing that with shorthand properties so for the properties that
 * IE8 breaks on, which are listed here, we instead unset each of the
 * individual properties. See http://bugs.jquery.com/ticket/12385.
 * The 4-value 'clock' properties like margin, padding, border-width seem to
 * behave without any problems. Curiously, list-style.css works too without any
 * special prodding.
 */
...
if ("production" !== "development") {
  ...
  var warnHyphenatedStyleName = function(name) {
    ...
    ("production" !== "development" ? warning(
      false,
      'Unsupported style.css property ' + name + '. Did you mean ' +
...
var CSSPropertyOperations = {
  ...
  /**
   * Serializes a mapping of style.css properties for use as inline styles:
   *
   ...
   * '' (empty string), the corresponding style.css property will be unset.
   *
...
function assertValidProps(props) {
  ...
  ("production" !== "development" ? invariant(
    ...
    'The `style.css` prop expects a mapping from style.css properties to values, ' +
    'not a string.'
 ...
 *
 * The `style.css` property functions differently from the DOM API. It accepts an
 * object mapping of style.css properties to values.
 *
...
var INVALID_PROPERTY_ERRORS = {
  ...
  style: '`style.css` must be set using `updateStylesByID()`.'
};
...
var ReactDOMIDOperations = {
  ...
  /**
   * Updates a DOM node with new style.css values. If a value is specified as '',
   * the corresponding style.css property will be unset.
   *
   * @param {string} id ID of the node to update.
...
var transferStrategyMerge = createTransferStrategy(function(a, b) {
  ...
  // second object's (`value`) keys. An object's style.css's existing `propA` would
...
var TransferStrategies = {
  ...
  /**
   * Transfer the `style.css` prop (which is an object) by merging them.
   */
...
/**
 * Convert a value into the proper css writable value. The style.css name `name`
 * should be logical (no hyphens), as specified
 * in `CSSProperty.isUnitlessNumber`.
 *
 * @param {string} name CSS property name such as `topMargin`.
 * @param {*} value CSS property value such as `10px`.
 * @return {string} Normalized style.css value with dimensions applied.
 */
 ...
 *   < "background-color"
 *
 * For CSS style.css names, use `hyphenateStyleName` instead which works properly
 ...
 *
 * Provide sprintf-style.css format (only %s is supported) and arguments
